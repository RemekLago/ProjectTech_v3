...
//        });
        ...
        String checkStatus = tmpdata5.get("Status");
        String checkPozycja = gettingMagazynLokalzacja();
        Log.i("checking", "checkStatus: " + checkStatus);
        Log.i("checking", "checkPozycja: " + checkPozycja);
        ...
        else if (checkStatus.equals(null)){
            Toast.makeText(getApplicationContext(), "Wpisz numer pozycji lub zeskanuj", Toast.LENGTH_LONG).show();
        }
...
//                editTextNumber.setText(null); // textview form Activity EnterData
                if( searchCode.length() > 20) {
                    Toast.makeText(getApplicationContext(), "Nieprawidłowa nazwa Magazyn/Lokalizacja", Toast.LENGTH_LONG).show();
                }
